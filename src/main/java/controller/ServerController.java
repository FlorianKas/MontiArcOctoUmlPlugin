...
/**
 * Forward changes in the graph to all clients. Used by MainController for
 * turning the diagram in to a host for clients to connect to.
 */
public class ServerController implements PropertyChangeListener {
  ...
  /**
   * Handles property changes in the model and forwards them to all connected
   * clients.
   */
  @Override
  public void propertyChange(PropertyChangeEvent evt) {
    ...
    if (propertyName.equals(Constants.sketchAdd)) {
      ...
      String[] dataArray = { Constants.changeSketchPoint, sketch.getId(), Double.toString(point.getX()), Double.toString(point.getY()) };
      server.sendToAllTCP(dataArray);
    }
    else if (propertyName.equals(Constants.changeSketchStart)) {
      Sketch sketch = (Sketch) evt.getSource();
      Point2D point = (Point2D) evt.getNewValue();
      String[] dataArray = { Constants.changeSketchStart, sketch.getId(), Double.toString(point.getX()), Double.toString(point.getY()), sketch.getColor().toString() };
      server.sendToAllTCP(dataArray);
    }
    ...
    else if (propertyName.equals(Constants.changeNodeTranslateX) || propertyName.equals(Constants.changeNodeTranslateY)) { // NodeX/Y
                                                                                                                           // not
                                                                                                                           // needed
