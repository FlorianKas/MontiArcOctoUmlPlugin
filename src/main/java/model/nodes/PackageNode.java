...
public class PackageNode extends AbstractNode {
  ...
  public AbstractNode findNode(Point2D point) {
    for (AbstractNode node : childNodes) {
      if (point.getX() >= node.getX() && point.getX() <= node.getX() + node.getWidth() && point.getY() >= node.getY() && point.getY() <= node.getY() + node.getHeight()) {
        return node;
      }
    }
    return null;
  ...
  public PackageNode copy() {
    ...
    /*
     * if(this.getChildNodes() != null){ for(AbstractNode child :
     * this.getChildNodes()){ newCopy.addChild(child.copy()); } }
     */
    return newCopy;
  }
