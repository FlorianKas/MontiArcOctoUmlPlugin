...
public class Graph implements Serializable, PropertyChangeListener {
  ...
  /**
   * Adds a Node to the Graph. If the node is added inside a package, the node
   * is also added as a child to that package.
   * 
  ...
  public void addNode(AbstractNode n, boolean remote) {
    ...
    for (AbstractNode node : allNodes) {
      if (node instanceof PackageNode) {
        if (n.getX() >= node.getX() && n.getX() <= node.getX() + node.getWidth() && n.getY() >= node.getY() && n.getY() <= node.getY() + node.getHeight()) {
  ...
  public boolean hasEdge(Edge edge) {
    ...
    for (Edge e : allEdges) {
      if ((e.getStartNode().equals(edge.getStartNode()) || e.getEndNode().equals(edge.getStartNode())) && (e.getEndNode().equals(edge.getEndNode()) || e.getStartNode().equals(edge.getEndNode()))) {
        return true;
      }
    }
    return false;
