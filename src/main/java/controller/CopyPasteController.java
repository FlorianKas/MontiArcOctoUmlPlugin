...
public class CopyPasteController {
  ...
  void setUpCopyCoords() {
    ...
    for (AbstractNode node : currentlyCopiedNodes) {
      if (node != closest) {
        copyDeltas.put(node, new double[] { node.getTranslateX() - closest.getTranslateX(), node.getTranslateY() - closest.getTranslateY() });
      }
      else {
  ...
  void paste() {
    ...
    // If a node has several edges it will be found more than once in the
    // currentlyCopiedEdges loop, so we put nodes
    ...
    for (AbstractEdge oldEdge : currentlyCopiedEdges) {
      for (AbstractNode node : currentlyCopiedNodes) {
        if (node.equals(oldEdge.getStartNode())) {
          if (!alreadyCopiedNodes.containsKey(node)) { // If start node is not
                                                       // already copied
          ...
          if (!alreadyCopiedNodes.containsKey(node)) { // If end node is not
                                                       // already copied
